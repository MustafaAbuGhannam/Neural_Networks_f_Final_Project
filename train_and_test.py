...
if __name__ == '__main__':
    writer_ = SummaryWriter('runs/without_reg_for_160_writers_on_english_on_ResNet18_lr_0.001')
    flag = 'Contrastive1'
    train_acc = []
    test_acc = []
    model = None
    ...
    for i in range(epochs):
        ...
        if flag == 'Contrastive':
            trainContrastive(model, loss_function, optimizer, train_line_data_loader, loss_history)
        else:
        ...
        if flag == 'Contrastive':
            testContrastive(model, train_line_data_loader_for_test, acc_history= [], all_acc_train=train_acc, all_acc_test= test_acc, train_flag=True, y_pred = y_pred, y_true = y_true)
        else:
        ...
        if flag == 'Contrastive':
            testContrastive(model, test_line_data_loader, acc_history= [], all_acc_train=train_acc, all_acc_test= test_acc, train_flag=False, y_pred = y_pred, y_true = y_true)
        else:
            test(model, test_line_data_loader, acc_history= [], all_acc_train=train_acc, all_acc_test= test_acc, train_flag=False, y_pred = y_pred, y_true = y_true)
